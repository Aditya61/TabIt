<script>
    ...
    onMount(() => {
        ...
        const handleBack = (event) => {
            ...
            goto(`/creditors/dashboard/${data.creditor.id}/credit/${data.credit.id}`);
        };
    ...
    // Pre-fill with the credit's current items so existing rows can be
    // edited in place, removed, or new rows added alongside them
    let items = data.credit.items.map((item) => ({
        name: item.name,
